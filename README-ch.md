...
<p align="center">
  <h1 align="center">什么是 <br /><img src="https://i.imgur.com/P1VL4bC.png" height="80" alt="p3g logo"/><br />？</h1>
</p>

**p3g** 是一个具有**全局缓存**、**自动链接**、**迷你工作空间**和**即时同步模式**的依赖管理器 — 100% 使用 **Bun + TypeScript** 构建。
...
**p3g** 通过在你的系统上创建一个**全局工作空间**来解决这个问题，依赖项只安装一次，通过*符号链接*（或复制，如果你愿意）重复使用。
...
**p3g** 创建一个全局仓库（`~/.p3g_workspace/js`）并将包直接链接到项目 — 就像一个依赖大脑。
...
用法：
  p3g axios@latest   → 直接安装包
  p3g use api        → 使用保存的迷你工作空间
  p3g list           → 列出迷你工作空间
  p3g --dev          → 作为 devDependency 安装
  p3g --copy         → 复制而不是链接
  p3g sync           → 复制整个全局工作空间
  p3g --verbose      → 详细日志
```
...
**Full Agentic Stack**、**EnzyChop.Tech**、**Virion.Delivery** 生态系统的创造者，现在还有… **p3g**。
